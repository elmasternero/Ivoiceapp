...
class InterfazApp:
    def __init__(self, ventana):
        ...
        # Botones
        self.boton_ver_bd = tk.Button(ventana, text="Ver Base de Datos", command=self.ver_bd,width=button_width)
        self.boton_insertar_factura = tk.Button(ventana, text="Insertar Factura", command=self.insertar_factura,width=button_width)
        self.boton_modificar_factura = tk.Button(ventana, text="Modificar Factura", command=self.modificar_factura,width=button_width)
        self.boton_salir = tk.Button(ventana, text="Salir", command=self.ventana.quit,width=button_width)
    ...
    def ver_bd(self):
        ...
        ventana_ver_bd.title("Visualizar Base de Datos")
    ...
    # Configurar los widgets de filtro en la parte superior de la columna 0
        opciones_filtro = ["Ver todo", "ID", "provider", "amount", "date", "yield"]
        self.filtro = tk.StringVar(value=opciones_filtro[0])
        tk.Label(ventana_ver_bd, text="Filtrar por:").grid(row=0, column=0, sticky="ew")
        ...
        tk.Button(ventana_ver_bd, text="Filtrar", command=self.aplicar_filtro).grid(row=3, column=0, sticky="ew")
    ...
    # Botón para volver al menú principal en la última fila
        tk.Button(ventana_ver_bd, text="Volver al Menú Principal", command=lambda: self.volver_al_menu_principal(ventana_ver_bd)).grid(row=4, column=0, sticky="ew")


    def aplicar_filtro(self):
        filtro = self.filtro.get()
        valor = self.valor_filtro.get() if self.filtro.get() != "Ver todo" else None
    ...
    def insertar_factura(self):
        ...
        ventana_insertar_factura.title("Insertar Factura")
        ...
        label_proveedor = Label(ventana_insertar_factura, text="Nombre del Proveedor:")
        label_monto = Label(ventana_insertar_factura, text="Monto:")
        label_mes_pago = Label(ventana_insertar_factura, text="Mes de Pago (YYYY-MM):")
        label_mes_correspondiente = Label(ventana_insertar_factura, text="Mes Correspondiente (0,1,2,3):")
        ...
        button_width = 20  # Define el ancho deseado para los botones
        boton_insertar = Button(ventana_insertar_factura, text="Insertar", command=on_insert, width=button_width)
        boton_menu_principal = Button(ventana_insertar_factura, text="Volver al Menú Principal", command=lambda: self.volver_al_menu_principal(ventana_insertar_factura), width=button_width)
    ...
    def modificar_factura(self):
        ...
        ventana_modificar_factura.title("Modificar Factura")
        ...
        Label(ventana_modificar_factura, text="ID de la Factura:").grid(row=0, column=0)
        ...
        Label(ventana_modificar_factura, text="Campo a modificar:").grid(row=1, column=0)
        #campos = ["provider", "amount", "date", "yield"]
        ...
        Label(ventana_modificar_factura, text="Nuevo Valor:").grid(row=2, column=0)
        ...
        Button(ventana_modificar_factura, text="Modificar Factura", command=modify_action).grid(row=3, column=0, padx=10, pady=10)
...
# )).grid(row=3, column=0, padx=10, pady=10)
        ...
        Button(ventana_modificar_factura, text="Volver al Menú Anterior", command=lambda: self.volver_al_menu_principal(ventana_modificar_factura)).grid(row=3, column=1,padx=10,pady=10)
